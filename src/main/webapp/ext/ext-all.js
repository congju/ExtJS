/*
 * Ext JS Library 3.2.1 Copyright(c) 2006-2010 Ext JS, Inc. licensing@extjs.com
			...
			if (typeof m == "object") {
				...
				for (n in l) {
					...
					if (!p.filterOptRe.test(n)) {
						p.addListener(n, t.fn || t, t.scope || l.scope, t.fn
										? t
										: l)
					}
				}
...
Ext.Element.addMethods(function() {
	var i = {}, z = /(-[a-z])/gi, c = {}, t = document.defaultView, w = Ext.isIE
			? "styleFloat"
		...
		for (j = 0; j < n; j++) {
			...
			if (q.anchor) {
				...
				if (!s) {
					...
					q.anchorSpec = s = {
						right : this
								.parseAnchor(u[0], q.initialConfig.width, d),
						bottom : this.parseAnchor(u[1], q.initialConfig.height,
								r)
					}
				}
				g = s.right ? this.adjustWidthAnchor(s.right(t)
								- e.getMargins("lr"), q) : undefined;
				b = s.bottom ? this.adjustHeightAnchor(s.bottom(l)
...
Ext.layout.VBoxLayout = Ext.extend(Ext.layout.BoxLayout, {
	...
	updateInnerCtSize : function(b, d) {
		var c = b.height, a = d.meta.maxWidth + this.padding.left
				+ this.padding.right;
	...
	calculateChildBoxes : function(w, e) {
		var k = w.length, u = this.padding, y = u.top, p = u.left, q = y
				+ u.bottom, a = p + u.right, x = e.width - this.scrollOffset, v = e.height, A = Math
		...
		for (B = 0; B < k; B++) {
			...
			switch (this.align) {
				case "stretch" :
					D = A - G;
					s.width = D.constrain(h.minWidth || 0, h.maxWidth
									|| 1000000);
					s.dirtySize = true;
					break;
				case "stretchmax" :
...
Ext.layout.HBoxLayout = Ext.extend(Ext.layout.BoxLayout, {
	...
	type : "hbox",
	updateInnerCtSize : function(b, d) {
		var a = b.width, c = d.meta.maxHeight + this.padding.top
				+ this.padding.bottom;
		if (this.align == "stretch") {
			c = b.height
		} else {
	...
	calculateChildBoxes : function(z, e) {
		var n = z.length, x = this.padding, C = x.top, r = x.left, s = C
				+ x.bottom, a = r + x.right, B = e.width - this.scrollOffset, y = e.height, h = Math
		...
		for (E = 0; E < n; E++) {
			...
			switch (this.align) {
				case "stretch" :
					t = h - j;
					v.height = t.constrain(l.minHeight || 0, l.maxHeight
									|| 1000000);
					v.dirtySize = true;
					break;
				case "stretchmax" :
...
Ext.Window = Ext.extend(Ext.Panel, {
			...
			focus : function() {
				...
				if (Ext.isDefined(a)) {
					...
					if (d && b) {
						if (!Ext.lib.Region.getRegion(b)
								.contains(Ext.lib.Region.getRegion(d.dom))) {
							return
						}
					}
				}
...
Ext.list.ColumnResizer = Ext.extend(Ext.util.Observable, {
	...
	onEnd : function(m) {
		...
		var j = this.hdIndex, o = this.view, h = o.columns, l = h.length, q = this.view.innerHd
				...
				.ceil((k * o.maxWidth) / q), p = (h[j].width * 100) - r, a = Math
				.floor(p / (l - 1 - j)), n = p - (a * (l - 1 - j));
		for (var d = j + 1; d < l; d++) {
...
Ext.form.ComboBox = Ext.extend(Ext.form.TriggerField, {
	...
	expand : function() {
		...
		this.list.alignTo.apply(this.list, [this.el].concat(this.listAlign));
		var b = Ext.getDom(this.getListParent() || Ext.getBody()), a = parseInt(
				Ext.fly(b).getStyle("z-index"), 10);
		if (!a) {
...
Ext.form.CompositeField = Ext.extend(Ext.form.Field, {
			...
			initComponent : function() {
				...
				this.fieldErrors = new Ext.util.MixedCollection(true, function(
								g) {
							return g.field
						});
				this.fieldErrors.on({
				...
				Ext.form.CompositeField.superclass.initComponent.apply(this,
						arguments)
			},
			onRender : function(d, b) {
				if (!this.el) {
...
(function		() {
			...
			onResize : function(e, c, a, d) {
				...
				Ext.form.CompositeField.superclass.onResize.apply(this,
						arguments)
			},
...
Ext.grid.GridView = Ext.extend(Ext.util.Observable, {
	...
	doRender : function(d, v, m, a, r, t) {
		...
		for (var s = 0, w = v.length; s < w; s++) {
			...
			for (var u = 0; u < r; u++) {
				...
				q.attr = q.cellAttr = "";
				q.style = h.style;
				q.value = h.renderer.call(h.scope, g.data[h.name], q, g, p, u,
						m);
	...
	renderUI : function() {
		var c = this.templates, e = this.renderHeaders(), a = c.body.apply({
					rows : "&#160;"
				});
		...
		if (d.enableColumnResize !== false) {
			this.splitZone = new Ext.grid.GridView.SplitDragZone(d,
					this.mainHd.dom)
		}
...
Ext.grid.CheckboxSelectionModel = Ext.extend(Ext.grid.RowSelectionModel, {
			...
			handleMouseDown : function() {
				Ext.grid.CheckboxSelectionModel.superclass.handleMouseDown
						.apply(this, arguments);
